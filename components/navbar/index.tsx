...
const Navbar = () => {
    ...
    const projectId = rawProjectId && rawProjectId !== "null" ? (rawProjectId as Id<"projects">) : null;
    ...
    const tabs: TabProps[] = [
        {
            label: "Canvas",
            href: `/dashboard/${me.name}/canvas?project=${project?._id}`,
            icon: <Hash className="w-4 h-4" />
        },
        {
            label: "Style Guide",
            href: `/dashboard/${me.name}/style-guide?project=${project?._id}`,
    ...
    const isProjectMode = hasCanvas || hasStyleGuide;

    return (
        <div className={cn(
            "grid grid-cols-2 lg:grid-cols-3 p-6 fixed top-0 left-0 right-0 z-50",
            !isProjectMode && "grid grid-cols-2 lg:grid-cols-2 p-6 fixed top-0 left-0 right-0 z-50"
        )}>
